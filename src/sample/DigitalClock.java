 ...
 * todo: usage as a timer, counting backwards
 * todo: setting time
 */
...
public class DigitalClock extends Application {
    ...
    public void start(Stage window) {
        ...
        //375 is scene.width
        window.setX(screenBounds.getWidth()-377);
        window.setY(2);
         ...
         */
        Scene scene = new Scene(root, 375, 90); //width, height

        // add background image
        BackgroundImage bg = new BackgroundImage
                (new Image("board.png", 180, 145, true, true),
                        BackgroundRepeat.REPEAT, BackgroundRepeat.NO_REPEAT, BackgroundPosition.CENTER, BackgroundSize.DEFAULT);
        // add digital clock
        Clock clock = new Clock(Color.rgb(0,75,235), Color.rgb(5, 15, 35)); //rgb is background digit color
        clock.setLayoutX(55);
        clock.setLayoutY(66);
        clock.secondTimeline();
        ...
        window.setOpacity(0.90); //less is less
        window.show();
    }
